    ...
    <div class="content-wrap">
      <ul class="list">
        <li class="item" @click="selectStyle(item.orderCode)" v-for="(item, index) in data" >
          ...
          <div class="address">
            <i class="iconfont iconjutidizhi"></i>
            <span class ="stateColor"  v-if="item.status =='待验货'">任务状态：{{item.status}}</span>
            <span class ="stateColorRed"  v-if="item.status =='已验货'">任务状态：{{item.status}}</span>
          </div>
          <div class="more">
            <span>详情信息</span>
...
<script>
    export default {
        ...
        created() {
            ...
            if(localStorage.getItem("pkyh"+userNsCode+"inputValiue") != null){
                inputValiue = localStorage.getItem("pkyh"+userNsCode+'inputValiue');
                this.params.customerName = inputValiue;
            }
            if(localStorage.getItem("pkyh"+userNsCode+"type") != null){
                type = localStorage.getItem("pkyh"+userNsCode+'type');
                let typeCode = localStorage.getItem("pkyh"+userNsCode+"typeCode");
        ...
        methods: {
            ...
            getData(inputValiue,type) {
                ...
                let userInfo = JSON.parse(localStorage.getItem('user'));
                let userNsCode = userInfo.userReal;
                this.$methods.http("wx/sys/sys/dict/conSearchResult", {inputValue:inputValiue,userNsCode:userNsCode,type:type,formType:'5'}).then(res => {
            ...
            search(event) {
                ...
                if(inputValiue ==''){
                    if(localStorage.getItem("pkyh"+userNsCode+"inputValiue") != null){
                        localStorage.removeItem("pkyh"+userNsCode+'inputValiue');
                        this.params.customerName = "";
                    }
                    if(localStorage.getItem("pkyh"+userNsCode+"type") != null){
                        localStorage.removeItem("pkyh"+userNsCode+"typeCode");
                        this.stateForm.state= "";
                    }
                }else{
                    localStorage.setItem("pkyh"+userNsCode+"inputValiue",inputValiue);
                    localStorage.setItem("pkyh"+userNsCode+"type",type);
                }
